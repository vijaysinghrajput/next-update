...
if (!function_exists('getAppUrl')) {
    function getAppUrl() {
        ...
        // Force production URL for production environment
        if (getEnvironment() === 'production') {
            return 'https://next-update.skyablyitsolution.com';
        }
...
if (!function_exists('getEnvironment')) {
    function getEnvironment() {
        ...
        if (strpos($host, 'localhost') !== false || strpos($host, '127.0.0.1') !== false || strpos($host, '192.168.') !== false) {
            ...
            return 'staging';
        } elseif (strpos($host, 'skyablyitsolution.com') !== false) {
            return 'production';
        } else {
            return 'production';
        }
...
return [
    ...
    'database' => [
        'host' => $environment === 'production' ? 'localhost' : 'localhost',
        'username' => $environment === 'production' ? 'u715885454_next_update' : 'root',
        'password' => $environment === 'production' ? 'Next@411' : '',
        'database' => $environment === 'production' ? 'u715885454_next_update' : 'local_news_app',
        'charset' => 'utf8mb4'
    ],
